...
DEFAULT_AGENTS = [
    # 管理工具
    {
        "id": "general",
        "name": "AI 助手",
        "description": "通用 AI 助手，可以帮你完成各种任务",
        "icon": "✦",
        "icon_bg_color": "#1a73e814",
        "icon_color": "#1a73e8",
        "category": "management",
    ...
    },
    {
        "id": "couch",
        "name": "智能体引导教练",
        "description": "通过引导式对话帮你理清思路",
        "icon": "🎯",
        "icon_bg_color": "#6366f114",
        "icon_color": "#6366f1",
        "category": "management",
    ...
    {
        "id": "ecommerce_consultant",
        "name": "电商商业顾问",
        "description": "电商领域的商业策略咨询",
        "icon": "👥",
        "icon_bg_color": "#6366f114",
        "icon_color": "#6366f1",
        "category": "management",
    ...
    # 设计工具
    {
        "id": "agent_writing",
        "name": "文案大师",
        "description": "专业文案撰写，涵盖广告文案、产品描述、品牌故事等",
        "icon": "✍️",
        "icon_bg_color": "#f59e0b14",
        "icon_color": "#f59e0b",
        "category": "design",
    ...
    },
    {
        "id": "agent_coder",
        "name": "编程助手",
        "description": "代码编写、调试、重构、技术方案设计",
        "icon": "💻",
        "icon_bg_color": "#22c55e14",
        "icon_color": "#22c55e",
        "category": "management",
...
DELETED_AGENTS_FILE = DATA_DIR / "deleted_agents.json"

DEFAULT_AGENT_IDS = {a["id"] for a in DEFAULT_AGENTS}


def _get_deleted_ids() -> set[str]:
    if DELETED_AGENTS_FILE.exists():
        try:
            return set(json.loads(DELETED_AGENTS_FILE.read_text(encoding="utf-8")))
        except Exception:
            pass
    return set()
...
def _load_agents() -> dict[str, dict]:
    ...
    agents = {a["id"]: a for a in DEFAULT_AGENTS}
...
def _save_agents(agents: dict[str, dict]):
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    AGENTS_FILE.write_text(json.dumps(agents, ensure_ascii=False, indent=2), encoding="utf-8")


@router.get("/", response_model=list[AgentResponse])
async def list_agents(category: str | None = None, current_user: dict = Depends(get_current_user)):
    agents = _load_agents()
    ...
    if not is_admin:
        ...
        result = [a for a in result if not a.get("department_id") or a.get("department_id") == user_dept]
    if category:
        result = [a for a in result if a.get("category") == category]
    return [AgentResponse(**a) for a in result]
...
async def create_agent(agent: AgentCreate, current_user: dict = Depends(get_current_user)):
    ...
    if role_id not in ("role_super_admin", "role_admin"):
        raise HTTPException(status_code=403, detail="无权限创建智能体")
    agents = _load_agents()
...
async def update_agent(agent_id: str, agent: AgentCreate, current_user: dict = Depends(get_current_user)):
    role_id = current_user.get("role_id", "")
    if role_id not in ("role_super_admin", "role_admin"):
        raise HTTPException(status_code=403, detail="无权限修改智能体")
    agents = _load_agents()
    if agent_id not in agents:
        raise HTTPException(status_code=404, detail="智能体不存在")
    agent_data = agent.model_dump()
    agent_data["id"] = agent_id
    agent_data["created_at"] = agents[agent_id].get("created_at", datetime.now().isoformat())
...
async def delete_agent(agent_id: str, current_user: dict = Depends(get_current_user)):
    role_id = current_user.get("role_id", "")
    if role_id not in ("role_super_admin", "role_admin"):
        raise HTTPException(status_code=403, detail="无权限删除智能体")
    agents = _load_agents()
    if agent_id not in agents:
        raise HTTPException(status_code=404, detail="智能体不存在")
    del agents[agent_id]
    if agent_id in DEFAULT_AGENT_IDS:
        ...
        DELETED_AGENTS_FILE.write_text(
